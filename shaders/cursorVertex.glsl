...
void main()
{
	...
	if(n_model.x == 1.0 && n_model.y == 1.0) n_model = vec2(0.4, 0.7);
	else if(n_model.x == 1.0 && n_model.y == 0.0) n_model = vec2(0.6,0.4);
	else if(n_model.x == 0.0 && n_model.y == 1.0) n_model = vec2(0, 0.65);
	n_model = n_model * 2.0;
